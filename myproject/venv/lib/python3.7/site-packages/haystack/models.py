...
class SearchResult(object):
    ...
    def _get_model(self):
        if self._model is None:
            try:
                self._model = haystack_get_model(
    ...
    def _get_distance(self):
        ...
        if self._distance is None:
            ...
            if not hasattr(self, self._point_of_origin['field']):
                raise SpatialError(
                    "The field '%s' was not included in search results, so the distance could not be calculated." % self._point_of_origin['field'])
            ...
            self._distance = Distance(km=geopy_distance.distance(
                (po_lat, po_lng), (lf_lat, lf_lng)).km)
