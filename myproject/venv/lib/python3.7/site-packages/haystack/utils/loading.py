...
def load_backend(full_backend_path):
    ...
    if len(path_bits) < 2:
        raise ImproperlyConfigured(
            "The provided backend '%s' is not a complete Python path to a BaseEngine subclass." % full_backend_path)
...
def load_router(full_router_path):
    ...
    if len(path_bits) < 2:
        raise ImproperlyConfigured(
            "The provided router '%s' is not a complete Python path to a BaseRouter subclass." % full_router_path)
...
class ConnectionHandler(object):
    ...
    def __getitem__(self, key):
        ...
        self.thread_local.connections[key] = load_backend(
            self.connections_info[key]['ENGINE'])(using=key)
        return self.thread_local.connections[key]
...
class ConnectionRouter(object):
    ...
    def routers(self):
        if self._routers is None:
            ...
            router_list = getattr(
                settings, 'HAYSTACK_ROUTERS', default_routers)
        ...
        for app_mod in haystack_get_app_modules():
            ...
            for item_name, item in inspect.getmembers(search_index_module, inspect.isclass):
                if getattr(item, 'haystack_use_for_indexing', False) and getattr(item, 'get_model', None):
                    ...
                    class_path = "%s.search_indexes.%s" % (
                        app_mod.__name__, item_name)
        ...
        for fieldname, field_object in index.fields.items():
            if field_object.document is True:
                if field_object.index_fieldname != self.document_field:
                    raise SearchFieldError("All 'SearchIndex' classes must use the same '%s' fieldname for the 'document=True' field. Offending index is '%s'." % (
